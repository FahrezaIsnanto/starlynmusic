...
export default function RiwayatPendaftaran() {
  ...
  useEffect(() => {
    async function fetchRiwayatPendaftaran() {
      try {
        const response = await axios.get('https://api-goletkosan.herokuapp.com/pendaftaran', {
          params: {
            no_rm_pasien,
          },
        });
  ...
  return (
    <Layout>
      <div className="contentRiwayatPendaftaran">
        <img src="/images/music.png" className="contentRiwayatPendaftaranImg" alt="music"></img>
        {isLoading ? (
          <p>Harap Tunggu..</p>
        ) : (
